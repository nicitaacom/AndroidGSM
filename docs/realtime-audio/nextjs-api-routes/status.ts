...
export async function GET(request: NextRequest) {
  try {
    ...
    return NextResponse.json({ isAuthorized: isRecent, lastSeen: data.lastSeen, deviceToken, sims: data.sims ?? [], isServiceActive: data.isServiceActive ?? false, isTestActive: data.isTestActive ?? false })
  } catch (error) {
